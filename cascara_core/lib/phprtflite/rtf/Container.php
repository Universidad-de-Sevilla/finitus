...
class Container {
	...
	function writeText($text, &$font, &$parFormat, $replaceTags = true) {				
		...
		if (!empty($replaceTags)) {
			...
			$text = preg_replace("/<HOY[ ]*(\/)?[ ]*>/mi", "\\bullet ", $text);
...
/**
Version writeText jjmc@us.es
**/
function writeTextjjmc($text, &$font, &$parFormat, $replaceTags = true) {
                $text = str_replace('\\', '\\\\', $text);
                $text = str_replace("\r\n", "\r\n".'\par ', $text);
                ...
                if (!empty($replaceTags)) {
                        ...
                        $text = preg_replace("/<CHTIME[ ]*(\/)?[ ]*>/mi", "\\chtime ", $text);
                        $text = preg_replace("/<CHPGN[ ]*(\/)?[ ]*>/mi", "\\chpgn ", $text);
                        $text = preg_replace("/<TAB[ ]*(\/)?[ ]*>/mi", "\\par \pard\li1440 \pard\li720 { ", $text);
                        $text = preg_replace("/<BULLET[ ]*(\/)?[ ]*>/mi", "\\bullet ", $text);
                        $text = preg_replace("/<TAB2[ ]*(\/)?[ ]*>/mi", "\\par \pard\li1440 { ", $text);
                        $text = preg_replace("/<BORRARP[ ]*(\/)?[ ]*>/mi", "\\par \\pard { ", $text);
